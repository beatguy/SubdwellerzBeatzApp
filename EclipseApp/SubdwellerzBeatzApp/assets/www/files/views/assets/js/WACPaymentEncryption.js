    ...
    var d = window.Crypto = {},
        k = d.util = {
            ...
            bytesToWords: function (b) {
                for (var a = [], c = 0, e = 0; c < b.length; c++, e += 8) a[e >>> 5] |= b[c] << 24 - e % 32;
                return a
            },
            ...
            bytesToBase64: function (b) {
                ...
                for (var a = [], c = 0; c < b.length; c += 3) for (var e = b[c] << 16 | b[c + 1] << 8 | b[c + 2], p = 0; p < 4; p++) c * 8 + p * 6 <= b.length * 8 ? a.push("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".charAt(e >>> 6 * (3 - p) & 63)) : a.push("=");
                return a.join("")
            },
...
function generateAuthorizationSignature(sharedSecret, applicationID, username, timestamp) {
	var message = (username + applicationID + timestamp);
	var hmacResult = Crypto.HMAC(Crypto.SHA256, message, sharedSecret, { asBytes: true });
	var base64result = base64_encode(hmacResult);
	return encodeHex(base64result);	
}

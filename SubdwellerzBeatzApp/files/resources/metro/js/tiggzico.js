            ...
            send: function () {
                ...
                if (options.type == "get") {
                    if (options.url.indexOf("?") == -1) {
                        options.url += "?";
                    } else if (!(options.url.indexOf("?") + 1 == options.url.length)) {
                        options.url += "&";
                    }
                    options.url += $t.util.param(options.data);
                }

                WinJS.xhr(options).then (
                    function (request) {
                        var response = (options.dataType.indexOf('json') !== -1) ? JSON.parse(request.responseText) : request.responseXML;
